...
export function Header() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const month = new Intl.DateTimeFormat('bg-BG', { month: 'long' }).format(now).toLowerCase();
      ...
      <div className="container mx-auto px-4 py-4 flex items-center justify-between relative">
        ...
        {mounted && (
          <div className="absolute left-1/2 transform -translate-x-1/2 text-center flex items-center gap-4">
            <span className="text-lg md:text-xl font-medium text-gray-700">{currentDateStr}</span>
            <a href={wikiUrl} target="_blank" rel="noopener noreferrer" className="text-lg md:text-xl font-medium text-blue-600 hover:underline" aria-label={`Wikipedia page for ${currentDateStr}`}>
              Wikipedia
            </a>
          </div>
        )}
